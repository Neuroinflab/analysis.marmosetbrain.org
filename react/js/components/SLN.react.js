...
export default class SLN extends React.Component {
    ...
    showTooltip({target_area, source_area, x, y} = {}) {
        ...
        let txt = getTargetAreaSummaryHTML(target_area, source_area, that._matrix, this.dataOverlay);
        tooltip.html(txt)
            .style("left", (x_) + "px")
            .style("top", (y_) + "px");
    }
    ...
    initialDraw() {
        ...
        let _left = 20;
        let colorFunction = getBlueColorFunc();
        ...
        let m = _.map(d3.range(size.y), function(v, i) {
            return _.map(d3.range(size.x), function(v, j) {
                let src = areas.get(sources[i]);
                let tar = areas.get(targets[j]);
                let elem = matrix.get_sln(tar, src);
        ...
        */
        ...
        function make_row(row) {
            ...
            //.data(row.filter(function(d) { return d.z > 0; }))
                ...
                .style("fill-opacity", function(d) { return d.sln >= 0 ? 1 : 1; })
    ...
    handleChange() {
        if (AppStore.getNodePicked()) {
            this.setState({
                ...
                dataOverlay: AppStore.getDataOverlay(),
            });
        } else {
            this.setState({
                dataOverlay: AppStore.getDataOverlay(),
                    ...
                    <section className="matrix">
                        <section id="graph-container">
                            ...
                            <div className="color-bar-container">
                                <ColorBar style="matrix" mode="SLN"/>
                            </div>
                        </section>
